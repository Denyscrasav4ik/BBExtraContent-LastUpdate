        ...
        private static void ReplacementNPCOperation(LevelBuilder __instance)
        {
            ...
            void ReplaceNpcsWithLevelObject(SceneObject ld)
            {
                ...
                for (int i = 0; i < metas.Length; i++)
                {
                    ...
                    if (co != null && co.GetReplacementNPCs() != null) // Replacement npcs will always be in this array. That's why there's no check for the npc replacing field.
                ...
                for (int i = 0; i < max; i++)
                {
                    ...
                    List<Character> npcsBeingReplaced = [.. data.GetReplacementNPCs().Where(x => __instance.Ec.npcsToSpawn.Any(z => z.Character == x))];
